...
- [ ] T043 Let the operator declare tunnel-local embedding hosts (for example a WireGuard/VPN interface address) through an environment variable read by `validate_endpoint` in `scripts/v4/embeddings.py`, accepting plain `http://` only to loopback or a declared host and still rejecting every other plaintext or verification-disabled endpoint before any text is sent; add the cases to `tests/v4/test_embeddings.py` per FR-026 and plan: Target Platform (partial)
- [ ] T044 Reconcile `scripts/artifact_publication.py` with `tests/test_artifact_publication.py`, which this feature added and which fails both tests against the committed shared publisher, so the v1–v3 publication tests pass while `scripts/v4/` keeps no dependency on it per plan: Constraints, v1–v3 behavior unchanged (partial)
- [ ] T045 Remove `scripts/persistence.py` from `manifest.REUSED` in `scripts/v4/manifest.py` (or declare it excluded), since no module under `scripts/v4/` imports it, keeping `verify()` passing per plan: Structure Decision, module registry (partial)
- [ ] T046 Review `scripts/v4/plots.py`, `tests/v4/test_plots.py`, and the `matplotlib` entry in the `v4` extra of `pyproject.toml`, which no artifact calls for: document the mean-metric chart CLI in the plan's structure and `quickstart.md`, or remove it (unrequested)
- [ ] T047 Review the `cost` block that `scripts/v4/retrieve.py` writes into run provenance (and `EmbeddingUsage` in `scripts/v4/embeddings.py`), which T030's provenance field list does not include: document its fields in `contracts/workflow-cli.md` beside the other provenance keys, or remove it (unrequested)
