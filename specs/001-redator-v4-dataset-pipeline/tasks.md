...
- [ ] T008 [US1] Wire the production source gate into `scripts/v4/run.py`, fix the production split to `test`, retain the existing `o200k_base` 512-token max/320 target/64 overlap profile, and record the source fingerprint, 1,000-row count, generation ID, policy, and exact output counters in `meta/manifest.json`.
...
- [ ] T022 [P] [US4] Add failing build/recovery tests in `tests/v4/test_index.py` for one embedding per published chunk, no other model index, `manifest.chunks` rows, compatible generation/model/revision/text profile, failed rebuild retaining the prior active build, and container restart persistence.
